        ...
        def attention(query):
            ...
            for a in xrange(num_heads):
                ...
                with variable_scope.variable_scope("Attention_l_%d" % a, dtype=dtype):
                    ...
                    '''
                    value = (math_ops.square(p_t - pos_i) * 2) / attn_local_D * attn_local_D
                    pre = math_ops.exp(math_ops.negative(value))
                    print(pre.get_shape())
                    pre = math_ops.cast(pre, tf.float32)
                    ai = math_ops.matmul(ai, pre)
                    '''
...
def local_align_attention_decoder(decoder_inputs,
                                  initial_state,
                                  attention_states,
                                  cell,
                                  attn_local_D=3,
...
def embedding_attention_decoder(decoder_inputs,
  ...
  """
    ...
    with variable_scope.variable_scope(
        ...
        return local_align_attention_decoder(
            emb_inp,
            initial_state,
            attention_states,
            cell,
